    ...
    <div style={{ minHeight: '100vh', background: '#121212', padding: '24px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px', animation: 'fadeIn .4s both' }}>
          <h2 style={{ color: '#FFD700', fontSize: '22px', fontWeight: 400, marginBottom: '6px' }}>Processando Requisição</h2>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px' }}>Conectando os pontos nos seus dados...</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div style={{ background: '#2C2C2C', borderRadius: '8px', border: '1px solid rgba(255,215,0,0.2)', padding: '24px', animation: 'slideInL .4s both' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#FFD700" strokeWidth="2" strokeLinecap="round" style={{ animation: 'spin 1s linear infinite' }}>
                <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
              </svg>
              <span style={{ color: '#fff', fontSize: '14px' }}>Log de Descoberta</span>
            </div>
            {STEPS.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '12px', opacity: i <= step ? 1 : 0.25, transition: 'opacity .3s' }}>
          ...
          <div style={{ background: '#2C2C2C', borderRadius: '8px', border: '1px solid rgba(255,215,0,0.2)', padding: '24px', animation: 'slideInR .4s both' }}>
              ...
              <svg width="100%" height="100%" viewBox="0 0 300 180">
                {CONNS.map(([a,b], i) => (
                  <line key={i} x1={NODES[a].cx} y1={NODES[a].cy} x2={NODES[b].cx} y2={NODES[b].cy}
                    stroke="#FFD700" strokeWidth="2" strokeDasharray="6 4"
                    opacity={showConns ? 0.5 : 0} style={{ transition: `opacity .4s ${i * 0.2}s` }}
                  />
                ))}
                {NODES.map((n, i) => (
                  <g key={n.id}>
                    <circle cx={n.cx} cy={n.cy} r={showNodes ? 20 : 0} fill="#FFD700" style={{ transition: `r .3s ${i * 0.15}s` }}/>
                    <text x={n.cx} y={n.cy} textAnchor="middle" dominantBaseline="middle" fill="#121212" fontSize="10" fontWeight="bold" opacity={showNodes ? 1 : 0} style={{ transition: `opacity .3s ${i * 0.15 + 0.2}s` }}>{n.id}</text>
                    <text x={n.cx} y={n.cy + (n.cy < 90 ? 30 : -30)} textAnchor="middle" fill="#FFD700" fontSize="11" opacity={showNodes ? 1 : 0} style={{ transition: `opacity .3s ${i * 0.15 + 0.2}s` }}>{n.label}</text>
                  </g>
                ))}
              </svg>
            </div>
            <div style={{ height: '3px', marginTop: '16px', background: 'linear-gradient(to right, transparent, #FFD700, transparent)', borderRadius: '2px', width: done ? '100%' : '0', transition: 'width 1s ease' }} />
            {done && (
              <button onClick={() => navigate('/editor')} style={{
                width: '100%', marginTop: '14px', background: '#FFD700', color: '#121212',
