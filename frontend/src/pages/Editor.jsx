...
function highlightSQL(line) {
  if (line.trim().startsWith('--')) {
    return <span style={{ color: '#6EE7B7' }}>{line}</span>
  }
  const parts = []
  ...
  while ((match = regex.exec(line)) !== null) {
    ...
    parts.push(<span key={match.index} style={{ color: '#10B981', fontWeight: 500 }}>{match[0]}</span>)
  ...
  const EXPORT_OPTS = [
    {
      ...
      action: async () => {
        ...
        try {
          ...
          a.href = url; a.download = 'bridgebi_query.m'; a.click()
          URL.revokeObjectURL(url)
          alert('✅ Arquivo baixado!\n\nNo Power BI Desktop:\n1. Obter Dados → Consulta Nula\n2. Clique em "Editor Avançado"\n3. Cole o arquivo .m\n4. Clique em Concluído')
        } catch { alert('Erro ao gerar arquivo. Verifique se o backend está rodando.') }
      }
    },
    {
      icon: Database,
      ...
      action: async () => {
        ...
        try {
          ...
          const url  = URL.createObjectURL(blob)
          const a    = document.createElement('a')
          a.href = url; a.download = 'bridgebi_data.xlsx'; a.click()
          URL.revokeObjectURL(url)
        } catch { alert('Erro ao gerar arquivo. Verifique se o backend está rodando.') }
      }
    },
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', transition: 'background .3s' }}>
      ...
      <div style={{ flex: 1, padding: '24px', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', animation: 'fadeIn .4s both' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <h2 style={{ color: 'var(--accent)', fontSize: '20px', fontWeight: 600 }}>Script Gerado</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '12px', marginTop: '2px' }}>Revise e exporte sua análise</p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--code-bg)', borderRadius: '10px', border: '1px solid var(--border)', overflow: 'hidden', animation: 'slideInL .4s both', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '20px', overflowX: 'auto' }}>
              {sql.split('\n').map((line, i) => (
                <div key={i} style={{ display: 'flex', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', lineHeight: '1.8' }}>
                  <span style={{ color: 'var(--text-muted)', width: '36px', flexShrink: 0, textAlign: 'right', paddingRight: '16px', userSelect: 'none', opacity: 0.5 }}>{i + 1}</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {result?.explanation && (
              <div style={{ background: 'var(--card)', borderRadius: '10px', border: '1px solid var(--border)', padding: '16px', animation: 'slideInR .4s both', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <p style={{ color: 'var(--accent)', fontSize: '12px', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '.5px', fontWeight: 600 }}>Sobre este script</p>
            ...
            <div style={{ background: 'var(--card)', borderRadius: '10px', border: '1px solid var(--border)', padding: '20px', animation: 'slideInR .4s .1s both', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <p style={{ color: 'var(--text)', fontSize: '14px', fontWeight: 500, marginBottom: '14px' }}>Validação</p>
              {CHECKS.map((c, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                  <div style={{ width: '16px', height: '16px', borderRadius: '3px', background: '#10B981', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ background: 'var(--footer-bg)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px', animation: 'fadeIn .4s .5s both' }}>
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', zIndex: 50, animation: 'fadeIn .2s both' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card)', borderRadius: '12px', border: '1px solid var(--border)', padding: '28px', width: '100%', maxWidth: '380px', animation: 'fadeUp .2s both', boxShadow: 'var(--shadow)' }}>
            ...
            <button onClick={() => setShowModal(false)} style={{ width: '100%', marginTop: '6px', background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '13px', cursor: 'pointer', padding: '8px', fontFamily: 'Inter, sans-serif', transition: 'color .2s' }}
